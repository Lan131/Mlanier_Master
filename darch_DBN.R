...
trainIndex <- createDataPartition(dat[,1], p = .7, 
                                   list = FALSE, 
                                   times = 1)
 DataTrain <- dat[ trainIndex , ]
 DataTest  <- dat[-trainIndex,]
  ...
  darch <- train(Species ~ ., data = dat, tuneLength = 12, trControl = tc, 
                 method = darchModelInfo(parameters, grid), preProc = c("center", "scale"),
                 darch.numEpochs = 15, darch.batchSize = 1000, testing = T)
...
darch <- train(Target ~ Cat1+Cat2+Cont1, data = DataTrain, tuneLength = 12, trControl = tc, preProc.targets = T,
               method = darchModelInfo(parameters, grid), preProc = c("center", "scale"), darch.isClass=F,
              darch.numEpochs = 15, darch.batchSize = 10, xvalid=DataTest[,3:5],yvalid=DataTest[,2])
...
darch <- darch(Target ~ Cat1+Cat2+Cont1, data = DataTrain,
               preProc.params = list(method = c("center", "scale")),
               preProc.targets = T,
               layers = c(1,20,50,20,1),
               darch.batchSize =  10,
               bp.learnRate = .01,
               darch.isClass = F,
               darch.numEpochs = 100, xValid=DataTest[,2:5],yValid=DataTest[,2],
               darch.unitFunction = linearUnit
)
